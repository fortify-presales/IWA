...
This will start a scan using the settings file `IWA-UI-Dev-Settings` and login macro `IWA-UI-Dev-Login.webmacro` files provided 
in the `etc` directory. It assumes the application is running on "localhost:8080". It will run a "Critical and High Priority" scan using the policy with id 1008.
